...
namespace StorageTest {
  ...
  [TestClass]
  public class SampleWithDictionaryTest {
    ...
    [TestMethod]
    public void TestSampleWithDictionary() {
      try {
    ...
    private int addData(int idInt, string? idString, string text) {
      var sample = new SampleWithDictionary(idInt, idString, text);
    ...
    private void update(int key, int idInt, string? idString, string text) {
      var sample = DC.Data.SampleWithDictionaries[key];
    ...
    private void assertDictionaries() {
      Assert.AreEqual(expectedIdInts.Count, DC.Data.SampleWithDictionariesByIdInt.Count);
      if (expectedIdInts.Count>0) {
        foreach (var expectedIntKVP in expectedIdInts) {
          Assert.AreEqual(expectedIntKVP.Value, 
            DC.Data.SampleWithDictionariesByIdInt[expectedIntKVP.Key].ToString());
        }
      }
      Assert.AreEqual(expectedIdStrings.Count, DC.Data.SampleWithDictionariesByIdString.Count);
      if (expectedIdStrings.Count>0) {
        foreach (var expectedIntKVP in expectedIdStrings) {
          Assert.AreEqual(expectedIntKVP.Value,
            DC.Data.SampleWithDictionariesByIdString[expectedIntKVP.Key].ToString());
        }
      }
    }
  }
