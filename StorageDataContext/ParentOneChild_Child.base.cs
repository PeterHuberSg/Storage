...
namespace StorageModel  {
  ...
  #region ParentOneChild_ChildWriter
  //      --------------------------

  /// <summary>
  /// Writes a CSV file containing records which can be read back as ParentOneChild_Child. Note that the keys of linked objects
  /// need to be provided in Write(), since the data context will not be involved.
  /// </summary>
  public class ParentOneChild_ChildWriter: IDisposable {
    ...
    /// <summary>
    /// Constructor, will write the ParentOneChild_Child header line into the CSV file. Dispose ParentOneChild_ChildWriter once done.
    /// </summary>
    public ParentOneChild_ChildWriter(string? fileNamePath, CsvConfig csvConfig, int maxLineCharLenght) {
      this.csvConfig = csvConfig;
      csvWriter = new CsvWriter(fileNamePath, csvConfig, maxLineCharLenght, null, 0);
      var csvHeaderString = Csv.ToCsvHeaderString(ParentOneChild_Child.Headers, csvConfig.Delimiter);
      csvWriter.WriteLine(csvHeaderString);
    }


    /// <summary>
    /// Writes the details of one ParentOneChild_Child to the CSV file
    /// </summary>
    public void Write(int key, string text, int parentKey, int? parentNullableKey) {
      if (key<0) {
        throw new Exception($"ParentOneChild_Child's key {key} needs to be greater equal 0.");
      }
      if (key<=lastKey) {
        throw new Exception($"ParentOneChild_Child's key {key} must be greater than the last written ParentOneChild_Child's key {lastKey}.");
      }
      lastKey = key;
      csvWriter.WriteFirstLineChar(csvConfig.LineCharAdd);
      csvWriter.Write(key);
      csvWriter.Write(text);
      if (parentKey<0) throw new Exception($"Cannot write parentOneChild_Child to CSV File, because Parent is not stored in DC.Data.ParentOneChild_Parents.");

      csvWriter.Write(parentKey.ToString());
      if (parentNullableKey is null) {
        csvWriter.WriteNull();
      } else {
        if (parentNullableKey<0) throw new Exception($"Cannot write parentOneChild_Child to CSV File, because ParentNullable is not stored in DC.Data.ParentOneChild_ParentNullables.");
    ...
    /// <summary>
    /// Executes disposal of ParentOneChild_ChildWriter exactly once.
    /// </summary>
    public void Dispose() {
      Dispose(true);
    ...
    /// <summary>
    /// Is ParentOneChild_ChildWriter already exposed ?
    /// </summary>
