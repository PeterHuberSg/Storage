...
#pragma warning restore 67
  ...
  #region ReadOnly_ChildWriter
  //      --------------------

  /// <summary>
  /// Writes a CSV file containing records which can be read back as ReadOnly_Child. Note that the keys of linked objects
  /// need to be provided in Write(), since the data context will not be involved.
  /// </summary>
  public class ReadOnly_ChildWriter: IDisposable {
    ...
    /// <summary>
    /// Constructor, will write the ReadOnly_Child header line into the CSV file. Dispose ReadOnly_ChildWriter once done.
    /// </summary>
    public ReadOnly_ChildWriter(string? fileNamePath, CsvConfig csvConfig, int maxLineCharLenght) {
      this.csvConfig = csvConfig;
      csvWriter = new CsvWriter(fileNamePath, csvConfig, maxLineCharLenght, null, 0);
      var csvHeaderString = Csv.ToCsvHeaderString(ReadOnly_Child.Headers, csvConfig.Delimiter);
      csvWriter.WriteLine(csvHeaderString);
    }


    /// <summary>
    /// Writes the details of one ReadOnly_Child to the CSV file
    /// </summary>
    public void Write(string text, int readOnlyParentKey, int? readOnlyParentNullableKey) {
      ...
      if (readOnlyParentKey<0) throw new Exception($"Cannot write readOnly_Child to CSV File, because ReadOnlyParent is not stored in DC.Data.ReadOnly_Parents.");

      csvWriter.Write(readOnlyParentKey.ToString());
      if (readOnlyParentNullableKey is null) {
        csvWriter.WriteNull();
      } else {
        if (readOnlyParentNullableKey<0) throw new Exception($"Cannot write readOnly_Child to CSV File, because ReadOnlyParentNullable is not stored in DC.Data.ReadOnly_ParentNullables.");
    ...
    /// <summary>
    /// Executes disposal of ReadOnly_ChildWriter exactly once.
    /// </summary>
    public void Dispose() {
      Dispose(true);
    ...
    /// <summary>
    /// Is ReadOnly_ChildWriter already exposed ?
    /// </summary>
