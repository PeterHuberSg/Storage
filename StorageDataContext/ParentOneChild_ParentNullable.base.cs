...
namespace StorageModel  {
  ...
  #region ParentOneChild_ParentNullableWriter
  //      -----------------------------------

  /// <summary>
  /// Writes a CSV file containing records which can be read back as ParentOneChild_ParentNullable. Note that the keys of linked objects
  /// need to be provided in Write(), since the data context will not be involved.
  /// </summary>
  public class ParentOneChild_ParentNullableWriter: IDisposable {
    ...
    /// <summary>
    /// Constructor, will write the ParentOneChild_ParentNullable header line into the CSV file. Dispose ParentOneChild_ParentNullableWriter once done.
    /// </summary>
    public ParentOneChild_ParentNullableWriter(string? fileNamePath, CsvConfig csvConfig, int maxLineCharLenght) {
      this.csvConfig = csvConfig;
      csvWriter = new CsvWriter(fileNamePath, csvConfig, maxLineCharLenght, null, 0);
      var csvHeaderString = Csv.ToCsvHeaderString(ParentOneChild_ParentNullable.Headers, csvConfig.Delimiter);
      csvWriter.WriteLine(csvHeaderString);
    }


    /// <summary>
    /// Writes the details of one ParentOneChild_ParentNullable to the CSV file
    /// </summary>
    public void Write(int key, string text) {
      if (key<0) {
        throw new Exception($"ParentOneChild_ParentNullable's key {key} needs to be greater equal 0.");
      }
      if (key<=lastKey) {
        throw new Exception($"ParentOneChild_ParentNullable's key {key} must be greater than the last written ParentOneChild_ParentNullable's key {lastKey}.");
      }
      lastKey = key;
    ...
    /// <summary>
    /// Executes disposal of ParentOneChild_ParentNullableWriter exactly once.
    /// </summary>
    public void Dispose() {
      Dispose(true);
    ...
    /// <summary>
    /// Is ParentOneChild_ParentNullableWriter already exposed ?
    /// </summary>
